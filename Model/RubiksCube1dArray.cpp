//
// Created by devan on 26-06-2025.
//
    ...
    void rotateFace(int ind) {
        ...
        for (int i = 0; i < 3 / 2; ++i) {
            for (int j = i; j < 3 - i - 1; ++j) {
                char temp = cube[getIndex(ind,i,j)];
                cube[getIndex(ind,i,j)] = cube[getIndex(ind,2-j,i)];
                cube[getIndex(ind,2-j,i)] = cube[getIndex(ind,2-i,2-j)];
                cube[getIndex(ind,2-i,2-j)] = cube[getIndex(ind,j,2-i)];
                cube[getIndex(ind,j,2-i)] = temp;
            }
        }
    }
